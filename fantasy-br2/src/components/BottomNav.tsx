...
const NAV_ITEMS: NavItem[] = [
  ...
  { id: 'lineup', label: 'Escalar', icon: <LineupIcon />, playerOnly: true },
  { id: 'draft', label: 'Draft', icon: <DraftIcon /> },
  { id: 'ranking', label: 'Ranking', icon: <RankIcon /> },
  { id: 'trades', label: 'Mercado', icon: <TradesIcon />, playerOnly: true },
  { id: 'chat', label: 'Chat', icon: <ChatIcon /> },
];

export default function BottomNav() {
  const { screen, setScreen, nickname, isAdmin, unreadCount, trades } = useStore();
  const pendingTrades = trades.filter(t => t.to === nickname && t.status === 'pending').length;

  const visibleItems = NAV_ITEMS.filter(item => {
    if (item.playerOnly && isAdmin) return false;
    return true;
  });
        ...
        {isAdmin && (
          <button
            ...
            className={`flex flex-col items-center justify-center gap-0.5 w-14 h-full transition-colors ${
              screen === 'admin' ? 'text-amber-400' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
            <AdminIcon />
